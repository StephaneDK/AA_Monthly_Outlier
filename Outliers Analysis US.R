...
saveWorkbook(wb, paste0("US Outliers - ",pred_date,".xlsx"), overwrite = T) 
...
# 
# 
# Sales_UK <- read.csv("US_AA_Data_COT.csv", header = T, stringsAsFactors = FALSE)
# Sales_UK <- 
#   Sales_UK %>%
...
# Sales_UK$ISBN <- as.character(Sales_UK$ISBN )
# 
# #Highlighting columns with inventory issues
# wb <- createWorkbook()
# addWorksheet(wb, sheetName="UK")
# writeData(wb, sheet="UK", x=Sales_UK)
# 
# 
# #auto width for columns
# width_vec <- suppressWarnings(apply(Sales_UK, 2, function(x) max(nchar(as.character(x)) + 1, na.rm = TRUE)))
# width_vec_header <- nchar(colnames(Sales_UK))  + 3
# max_vec_header <- pmax(width_vec, width_vec_header)
# setColWidths(wb, "UK", cols = 1:ncol(Sales_UK), widths = max_vec_header+2 )
# setColWidths(wb, "UK",  cols = 2, widths = 40)
# setColWidths(wb, "UK",  cols = 7, widths = 20)
# 
# 
# #adding filters
# addFilter(wb, "UK", rows = 1, cols = 1:ncol(Sales_UK))
# 
# 
# #Centering cells
# centerStyle <- createStyle(halign = "center")
# addStyle(wb, "UK", style=centerStyle, rows = 1:nrow(Sales_UK)+1, cols = 8:ncol(Sales_UK), 
#          gridExpand = T, stack = TRUE)
# 
# # Adding borders
# invisible(OutsideBorders(
